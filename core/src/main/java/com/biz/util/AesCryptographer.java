...
public class AesCryptographer {
     ...
     */
    public byte[] encrypt(byte[] content, String password, String ivParam) throws RuntimeException {
     ...
     * 使用AES 算法 加密，默认模式 AES/CBC/PKCS5Padding
     */
    public byte[] encrypt(String content, String password, String ivParam) throws RuntimeException {
    ...
    public byte[] decrypt(byte[] encryptedContent, String password, String ivParam) throws RuntimeException {
        try {
//         LogConfig.setLog("password:"+password+"  ivParam:"+ivParam);
//         LogConfig.setLog(Hex.encodeHex(encryptedContent, false));
            Cipher cipher = getCipher();
